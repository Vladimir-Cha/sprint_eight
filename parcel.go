...
func (s ParcelStore) Add(p Parcel) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("ошибка добавления посылки в базу: %w", err)
	}
	// верните идентификатор последней добавленной записи
	id, err := newParsel.LastInsertId()
	if err != nil {
		return 0, fmt.Errorf("не получил идентификатор последней добавленной записи: %w", err)
	}
	return int(id), nil
}
...
func (s ParcelStore) Get(number int) (Parcel, error) {
	...
	parcelNumberGet := s.db.QueryRow("SELECT number, client, status, address, created_at FROM parcel WHERE number = :number",
		sql.Named("number", number))
	// заполните объект Parcel данными из таблицы
	p := Parcel{}
	err := parcelNumberGet.Scan(
		&p.Number,
		&p.Client,
		&p.Status,
		&p.Address,
	...
	if err != nil {
		return Parcel{}, fmt.Errorf("ошибка получения данных посылки по номеру: %w", err)
	}
...
func (s ParcelStore) GetByClient(client int) ([]Parcel, error) {
	...
	defer parcelsClient.Close()

	// заполните срез Parcel данными из таблицы
	var res []Parcel
	for parcelsClient.Next() {
		p := Parcel{}

		err = parcelsClient.Scan(
			&p.Number,
			&p.Client,
			&p.Status,
			&p.Address,
